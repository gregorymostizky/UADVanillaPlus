...
- When several Codex sessions are contributing to the same VP feature, treat this repo/session as the single implementation, build, and game-copy owner. Other sessions may send findings, reviews, logs, or suggested patches as handoffs, but apply and verify the actual source changes here before building/copying so the game always receives one coherent DLL.
