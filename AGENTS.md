...
- When several Codex sessions are contributing to the same VP feature, keep exactly one dedicated builder session as the source-code, version-bump, build, game-copy, commit, push, and release owner. Thinker/reviewer sessions may freely update `plans/`, `AGENTS.md`, README notes, and other docs with findings, reviews, logs, suggested patches, and handoffs, but should not modify source code or build/copy DLLs unless the user explicitly promotes that session to the builder role.
